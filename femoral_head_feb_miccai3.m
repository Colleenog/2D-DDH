function [center_FH_new, radius_FH_new, quality_FH, quality_trirad] = femoral_head_feb_miccai3 (sps1,probability,map,radius_FH,center_FH,center_alpha_beta,alpha,beta,size_mm,im,map_min_bias, map_max_bias) 
...
%    end
    ...
    map_min = map_min + map_min_bias;
    map_max = map_max + map_max_bias;
...
% region_bw based on straight line between acetabulum and labrum
    straight_line = [m_F, c_F];

    imageSizeX = size(im,2);
    imageSizeY = size(im,1);
    [columnsInImage rowsInImage] = meshgrid(1:imageSizeX, 1:imageSizeY);

    linepixels = abs(rowsInImage - m_F*columnsInImage-c_F) <= 5;
    %imagesc(linepixels)
...
% hough based circle fit
    ...
    for i = 1:size(h,3)
        ...
        h_new(:,:,i) = h(:,:,i) .* region_bw .* linepixels;
